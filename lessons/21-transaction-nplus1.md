...
JPA에서 사람이 가장 많이 다치는 두 곳입니다. 하나는 **`@Transactional`이 안 먹는 줄도 모르고 도는 코드**, 다른 하나는 **쿼리 1번인 줄 알았는데 501번 나가는 코드**. 둘 다 조용히 동작하다 운영에서 터집니다.
...
```java
@Service public class MemberService {          // Java — 클래스는 기본이 open
    @Transactional public void register(...) { ... }
}
```

```kotlin
@Service class MemberService {                 // Kotlin — 기본이 final
    @Transactional fun register(...) { ... }   // = public final void register(...)
}
```
...
@Service class OrderService {
    @Transactional private fun doWork() { ... }   // 프록시가 못 가로챔 — 무시됨
}
```
...
class OrderService(private val repo: OrderRepository) {
    ...
    @Transactional fun save(request: OrderRequest) { ... }   // 트랜잭션 없이 실행됨
}
```
...
fun register(cmd: RegisterCommand) {
    try { repo.save(Member(cmd.name)) }
    catch (e: Exception) { log.error("실패", e) }   // 예외를 삼킴 → 롤백 없이 커밋
}
```
...
`readOnly = true`면 Hibernate가 **플러시 모드를 MANUAL로 바꿔 더티 체킹용 스냅샷을 만들지 않습니다.** 조회 결과가 많을수록 메모리·CPU 절약이 크고, DB에 따라 읽기 전용 복제본으로 라우팅되기도 합니다. 실무 관행은 **클래스에 걸고 쓰기 메서드만 덮어쓰기**예요.
...
class MemberService(private val repo: MemberRepository) {
    ...
    @Transactional fun register(cmd: RegisterCommand): Long = ...   // 여기만 쓰기
}
```
...
fun listOrders(): List<OrderResponse> {
    ...
    return orders.map { OrderResponse(it.id!!, it.member.name) }  // ← 주문 수만큼 쿼리
}
```

`order.member`는 `LAZY` 프록시입니다. `.name`을 건드리는 순간 초기화되며 `select * from member where id = ?`가 나갑니다. 주문이 500건이면 **1 + 500 = 501번.**

개발 DB에는 주문이 3건이라 4번, 아무도 못 느낍니다. 운영에서 트래픽이 붙으면 커넥션 풀이 마르고 API가 타임아웃 나요. **N+1은 성능 문제가 아니라 장애 원인**입니다.

`EAGER`로 바꾸면 더 나빠집니다. `findAll()`은 JPQL이라 EAGER도 즉시 조인하지 않고 **로딩 후 하나씩 추가 조회**해서 결국 같은 N+1이 되고, 필요 없는 쿼리까지 항상 나가요. **연관관계는 전부 `LAZY`**, 필요한 곳에서 명시적으로 함께 가져오는 게 답입니다.

### 해결 1 — fetch join

```kotlin
// Kotlin / Java 동일한 JPQL
@Query("select o from Order o join fetch o.member")
fun findAllWithMember(): List<Order>
```

한 번의 조인으로 다 가져옵니다. **주의: 컬렉션(`@OneToMany`) fetch join은 페이징과 함께 못 씁니다.** Hibernate가 `HHH000104` 경고를 찍고 **전체를 메모리에 올린 뒤 잘라내요.** 데이터가 많으면 OOM입니다. 로그에 이 경고가 보이면 즉시 고치세요.
...
JPQL 없이 선언적으로 fetch join을 겁니다. 메서드 이름 쿼리에도 붙일 수 있어 Spring Data와 궁합이 좋아요. 내부적으로 **left outer join**이라 fetch join과 같은 페이징 제약을 받습니다.
...
또는 전역으로 `spring.jpa.properties.hibernate.default_batch_fetch_size: 100`. 프록시를 초기화할 때 **`where id in (?, ?, ..., ?)`로 100개씩 모아** 가져옵니다. 1 + N이 **1 + ceil(N/100)**이 돼요. 페이징과 함께 쓸 수 있고 카테시안 곱도 안 생깁니다.
...
`org.hibernate.LazyInitializationException: could not initialize proxy [Member#1] - no Session`

영속성 컨텍스트(=Session)가 닫힌 뒤 LAZY 프록시를 건드리면 납니다. 컨텍스트의 생존 범위는 **기본적으로 트랜잭션 범위**라, `@Transactional` 메서드를 빠져나오면 준영속(detached)이 되거든요. 그런데 Spring Boot 기본값은 `spring.jpa.open-in-view=true` — **컨텍스트를 컨트롤러·뷰 렌더링까지 열어둡니다.** 그래서 서비스 밖에서 LAZY를 건드려도 그냥 동작해요. 편하지만 대가가 있습니다.
